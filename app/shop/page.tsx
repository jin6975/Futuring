...
const CHARGE_PACKAGES = [
  { id: 'starter',  label: '스타터팩',  pts: 10_000,     price: 10_000,   badge: '입문' },
  { id: 'basic',    label: '베이직팩',  pts: 50_000,     price: 50_000,   badge: 'BEST' },
  { id: 'premium',  label: '프리미엄팩', pts: 100_000,   price: 100_000,  badge: '인기' },
  { id: 'elite',    label: '엘리트팩',  pts: 500_000,    price: 500_000,  badge: '' },
  { id: 'master',   label: '마스터팩',  pts: 1_000_000,  price: 1_000_000, badge: '고래' },
  { id: 'whale',    label: '웨일팩',    pts: 10_000_000, price: 10_000_000, badge: '🐋' },
]

function useNow() {
  const [n, setN] = useState(0)
  useEffect(() => { setN(Date.now()); const t = setInterval(() => setN(Date.now()), 60000); return () => clearInterval(t) }, [])
  return n
}
...
export default function ShopPage() {
  const { walletBalance, customization, purchaseItem, equipItem, unequipCategory, currentUser } = usePledgeStore()
  ...
  const handleCharge = async (pkg: typeof CHARGE_PACKAGES[0]) => {
    if (!currentUser.userId) { alert('로그인이 필요해요'); return }
    ...
    try {
      ...
      const clientKey = process.env.NEXT_PUBLIC_TOSS_CLIENT_KEY ?? 'test_ck_D5GePWvyJnrK0W0k6q8gLzN97Eo8'
      const tossPayments = await loadTossPayments(clientKey)
      const payment = tossPayments.payment({ customerKey: currentUser.userId })
      ...
      await payment.requestPayment({
        ...
        orderId,
        orderName: `Futuring ${pkg.label} (${pkg.pts.toLocaleString()}P)`,
        successUrl: `${window.location.origin}/payment/success?packageId=${pkg.id}`,
        failUrl: `${window.location.origin}/payment/fail`,
        customerEmail: `${currentUser.username}@futuring-user.com`,
        customerName: currentUser.username,
      })
    } catch (e) {
      // 유저가 결제창 닫은 경우 포함
      console.log('[payment cancelled or error]', e)
    } finally {
      setPayLoading(false)
    }
  }
      ...
      <div style={isMobile ? { padding: '20px 16px 100px' } : { maxWidth: 1000, margin: '0 auto', padding: '40px 40px 60px' }}>
        ...
        {chargeTab && (
          <div style={{ background: 'linear-gradient(135deg,#4F46E5,#7C3AED)', borderRadius: 20, padding: 28, marginBottom: 24 }}>
            ...
            <p style={{ fontSize: 13, color: 'rgba(255,255,255,0.7)', marginBottom: 24 }}>토스페이먼츠로 안전하게 결제됩니다</p>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2,1fr)', gap: 12 }}>
              {CHARGE_PACKAGES.map(pkg => (
                <button key={pkg.id} onClick={() => handleCharge(pkg)} disabled={payLoading}
                  style={{ background: 'rgba(255,255,255,0.12)', border: '1.5px solid rgba(255,255,255,0.2)', borderRadius: 16, padding: '16px 14px', cursor: payLoading ? 'not-allowed' : 'pointer', textAlign: 'left' as const, position: 'relative' as const, opacity: payLoading ? 0.7 : 1 }}>
                  {pkg.badge && <span style={{ position: 'absolute', top: 8, right: 8, fontSize: 10, fontWeight: 700, background: '#F59E0B', color: '#fff', padding: '2px 6px', borderRadius: 99 }}>{pkg.badge}</span>}
                  <p style={{ fontSize: 13, fontWeight: 700, color: 'rgba(255,255,255,0.7)', marginBottom: 4 }}>{pkg.label}</p>
                  <p style={{ fontSize: 18, fontWeight: 900, color: '#fff' }}>{pkg.pts >= 10000 ? `${(pkg.pts / 10000).toFixed(0)}만P` : `${pkg.pts.toLocaleString()}P`}</p>
                  <p style={{ fontSize: 13, color: 'rgba(255,255,255,0.6)', marginTop: 4 }}>₩{pkg.price.toLocaleString()}</p>
                </button>
              ))}
            </div>
            <p style={{ fontSize: 12, color: 'rgba(255,255,255,0.5)', marginTop: 16, textAlign: 'center' as const }}>
              🔒 토스페이먼츠 보안 결제 · 카드/계좌이체 지원
            </p>
          </div>
        )}
